...
!declaration
double precision r(1:3,1:3,-8:1),v(1:3,1:3,-8:1),a(1:3,1:3,-8:1),m(1:3),dt,tcurr,tout,G
double precision r_newpast(1:3,1:3,-3:-1), v_newpast(1:3,1:3,-3:-1), a_newpast(1:3,1:3,-3:-1)  !these arrays store the new position, velocity and acceleration produced when the timestep is halved. They can be considered as t=-1/2 step
...
double precision r_c(1:3,1:3,1), v_c(1:3,1:3,1),a_c(1:3,1:3,1) !r_c, v_c, a_c store the future corrected values
double precision dr(1:3), rad2, AU, yr, com(1:3), cov(1:3), mtot, KE, PE, Ei, Ecurr, delE, rad, tcount, dsun(1:3)
double precision techeck, tdsun, day, relerr, small, err_fact_r(1:3,1:3), err_fact_v(1:3,1:3), err_max_r(1:3), err_max_v(1:3)
integer i,j,k,n,p
...
OPEN(3,file='xypos.txt',status='new')
do
    !x--------PREDICTOR---------x
    !Predicted position, r(t+1)
    r(1:3,1:n,1) = r(1:3,1:n,0) + (dt*(-9.*r(1:3,1:n,-3) + 37.*r(1:3,1:n,-2) - 59.*r(1:3,1:n,-1) + 55.*r(1:3,1:n,0)))/24. !coefficients are taken from the PREDICTOR-CORRECTOR METHOD PAPER
    !Predicted velocity, v(t+1)
    v(1:3,1:n,1) = v(1:3,1:n,0) + (dt*(-9.*v(1:3,1:n,-3) + 37.*v(1:3,1:n,-2) - 59.*v(1:3,1:n,-1) + 55.*v(1:3,1:n,0)))/24.
    !predicted acceleration, a(t+1)
    a(1:3,1:n,1) = a(1:3,1:n,0) + (dt*(-9.*a(1:3,1:n,-3) + 37.*a(1:3,1:n,-2) - 59.*a(1:3,1:n,-1) + 55.*a(1:3,1:n,0)))/24.
    !x--------CORRECTOR---------x
    ...
    r_c(1:3,1:n,1) = r(1:3,1:n,0) + (dt*(r(1:3,1:n,-2) - 5.*r(1:3,1:n,-1) + 19.*r(1:3,1:n,0) + 9.*r(1:3,1:n,1)))/24.
    !Corrected velocity, v_c(t+1)
    v_c(1:3,1:n,1) = v(1:3,1:n,0) + (dt*(v(1:3,1:n,-2) - 5.*v(1:3,1:n,-1) + 19.*v(1:3,1:n,0) + 9.*v(1:3,1:n,1)))/24.
    !Corrected acceleration, a_c(t+1)
    a_c(1:3,1:n,1) = a(1:3,1:n,0) + (dt*(a(1:3,1:n,-2) - 5.*a(1:3,1:n,-1) + 19.*a(1:3,1:n,0) + 9.*a(1:3,1:n,1)))/24.
    ...
    do i=1,n
        ...
        err_max_r(i) = max(err_fact_r(1,i),err_fact_r(2,i),err_fact_r(3,i)) !this finds the maximum error among the x,y,z components of the position and velocity for each object.
        err_max_v(i) = max(err_fact_v(1,i),err_fact_v(2,i),err_fact_v(3,i))
    end do

    !The maximum error among the objects is compared against the relative error. ????????????????????DOUBT????????????????????
    !This is just using the error determined for position.
    if(max(err_max_r(1),err_max_r(2),err_max_r(3))>relerr) Then
        dt = dt/2.
        !?????????????DOUBT IN THIS PART, WHERE WE INTRODUCE NEW PAST STEPS OF -3/2 AND -1/2.
        ...
        v_newpast(1:3,1:n,-1) = (-5.*v(1:3,1:n,-4) + 28.*v(1:3,1:n,-3) - 70.*v(1:3,1:n,-2) + 140.*v(1:3,1:n,-1) + 35.*v(1:3,1:n,0))/128.
        a_newpast(1:3,1:n,-1) = (-5.*a(1:3,1:n,-4) + 28.*a(1:3,1:n,-3) - 70.*a(1:3,1:n,-2) + 140.*a(1:3,1:n,-1) + 35.*a(1:3,1:n,0))/128.

        !This is the t=-3/2 step
        r_newpast(1:3,1:n,-3) = (3.*r(1:3,1:n,-4) - 20.*r(1:3,1:n,-3) + 90.*r(1:3,1:n,-2) + 60.*r(1:3,1:n,-1) - 5.*r(1:3,1:n,0))/128.
        v_newpast(1:3,1:n,-3) = (3.*v(1:3,1:n,-4) - 20.*v(1:3,1:n,-3) + 90.*v(1:3,1:n,-2) + 60.*v(1:3,1:n,-1) - 5.*v(1:3,1:n,0))/128.
        a_newpast(1:3,1:n,-3) = (3.*a(1:3,1:n,-4) - 20.*a(1:3,1:n,-3) + 90.*a(1:3,1:n,-2) + 60.*a(1:3,1:n,-1) - 5.*a(1:3,1:n,0))/128.
    ...
    else if(max(err_max_r(1),err_max_r(2),err_max_r(3))<(0.01*relerr)) then
        dt = dt*2.
        do p=-3,-1
            r(1:3,1:n,p) = r(1:3,1:n,2*p)  !here the t=-6 becomes the fourth past position in the predictor method in place of t=-3
            v(1:3,1:n,p) = v(1:3,1:n,2*p)
            a(1:3,1:n,p) = a(1:3,1:n,2*p)
            !????????????????DOUBT- GIVING A CHECK ON PAST STEPS GETTING EXHAUSTED???????????/
        end do
    end if
    ...
    tcurr = tcurr + dt
    write(3,*) tcurr/yr,r_c(1:2,1:n,1)/AU
    !------------------------------------------
    !The final past step (t=-8) is shifted out of the array, and the corrected value at t=+1 is shifted to t=0
    do p=-8,0
        r(1:3,1:n,p) = r(1:3,1:n,p+1)
        if(p==0) Then
            r(1:3,1:n,p) = r_c(1:3,1:n,p+1)  !This IF condition is introduced so that the corrected value at t=+1 is shifted to t=0.
        end if
    end do
