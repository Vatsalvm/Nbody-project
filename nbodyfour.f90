...
!declaration
double precision r(1:3,1:3,-8:1),v(1:3,1:3,-8:1),a(1:3,1:3,-8:1),m(1:3),dt,tcurr,textracount,tout,G
...
OPEN(3,file='xypos_dt500.txt',status='new')
!open(4,file='encheck_dt100.txt',status='new')
do
    !x--------PREDICTOR---------x
...
!    write(6,*) 'y corrected velocity of earth:',v_c(2,3,1)/1000.
    ...
    if(textracount==(0.16*yr)) then !a reading every 2 months
        write(3,*) tcurr/yr,r_c(1:2,1:n,1)  !writing the xy position (corrected) for the objects to a file
        textracount = 0.
    end if
!
