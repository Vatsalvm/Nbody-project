...
!open(5,file='encheck_new_1.txt',status='new')
!open(3,file='dsun_6.txt',status='new')
...
!    end if
    ...
    if(max(err_r,err_v)>relerr) Then  !finds the maximum error contribution from the position or velocity
        counter = 0.  !reset the counter which is used for checking if timestep is doubled is four times consecutively.
