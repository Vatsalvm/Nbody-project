...
!OPEN(3,file='xypos_3.txt',status='new')
!open(5,file='encheck_dt10_1000new.txt',status='new')
!open(3,file='dsun_4.txt',status='new')
...
!        write(6,*) 'r jup = ',r(1,2,-4:0)/AU
        ...
        do i=1,n
        !So the first four past steps taken in by the predictor will be: ???????????????DOUBT HER

            r(1:3,i,-4) = r(1:3,i,-2)   !the -3/2 step going into the -3 paststep
            v(1:3,i,-4) = v(1:3,i,-2)
            a(1:3,i,-4) = a(1:3,i,-2)
...
!        stop
    ...
    if(max(err_r,err_v)<(0.01*relerr)) then
        if(counter<4) then
            dt = dt*2.
...
!            write(6,*) 'r jup = ',r(1,2,-8:0)/AU
        ...
        !when counter reaches 4, doubling will not be possible
        !write(6,*) 'doubling the dt'

        else if(counter==4) then
            !write(6,*) 'doubling stops'
            counter = 0
        end if
    end if
    !-------------------------------------------------------
...
!write(6,*) '--------------------'
!write(6,*) 'after downshifting'
!write(6,*) 'v jup - v(2,2,-8:0)=',v(2,2,-8:0)/1000.
!write(6,*) '      - v_c(2,2,1)=',v_c(2,2,1)/1000.
!write(6,*) '--------------------'
    !write(6,*) 'dt=',dt
    if(tcurr>tout) exit
end do
